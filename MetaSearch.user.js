...
bt.addSource("Apollo", function () {
    var apollo = "https://apollo.rip/ajax.php?action=browse&searchstr={query}";

    return {
        website: "https://apollo.rip/",
        url: {
            all: apollo,
            music: apollo + "&filter_cat[1]=1",
            music_flac: apollo + "&filter_cat[1]=1&format=FLAC",
            //		music_mp3: apollo + "&filter_cat[1]=1&format=AAC|MP3",
            elearning: apollo + "&filter_cat[3]=1&filter_cat[4]=1&filter_cat[5]=1&filter_cat[7]=1",
            mags: apollo + "&filter_cat[3]=1",
            ebooks: apollo + "&filter_cat[3]=1",
            fiction: apollo + "&filter_cat[3]=1",
            abooks: apollo + "&filter_cat[4]=1",
            comics: apollo + "&filter_cat[7]=1",
            apps_win: apollo + "&filter_cat[2]=1"
        },
        ...
        onRender: function (data, table) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
				ed = ""; media = "";
                group = data[i];
                link = "https://apollo.rip/torrents.php?id=" + group.groupId;

                if ("torrents" in group) {
                    artist = (group.artist === 'Various Artists')
                        ? 'Various Artists'
                        : bt.ab.clone().attr("href", "https://apollo.rip/artist.php?id=" + group.torrents[0].artists[0].id).html(group.artist);
                    ...
                    for (n = 0, nl = group.torrents.length; n < nl; n++) {
                        ...
                        groupTableHTML += '<tr><td><a href="https://apollo.rip/torrents.php?torrentid=' + torrent.torrentId + '" target="_blank">' + torrent.format + ' / ' + torrent.encoding + score + cue + '</a>' + (torrent.isFreeleech ? ' <span class="label label-success">Freeleech</span>' : '') + '</td><td>' + torrent.seeders + '</td><td><a href="https://apollo.rip/torrents.php?action=download&id=' + torrent.torrentId + '">' + bt.humanizeSize(torrent.size) + '</a></td></tr>';
                    }
                    ...
                    table.append('<tr><td><a href="' + link + '" target="_blank">' + group.groupName + '</a></td><td>' + group.seeders + '</td><td><a href="https://apollo.rip/torrents.php?action=download&id=' + group.torrentId + '">' + bt.humanizeSize(group.size) + '</a></td></tr>');
                }
            }
        },
        icon: "data:image/jpeg;base64,"
    };
});
